...
| `hash_threads` | integer 1–1024 | *none* — as many threads as the machine has cores, capped at 8 | `--threads` | `MMM_HASH_THREADS` |
...
- **`hash_threads`** is how many files the duplicate scan may hash at once. It bounds the *storage device*, not the CPU — the hashing is fast and most of the time is spent waiting on reads, so the number is really a queue depth. High counts help on NVMe; on a spinning disk, which has one head, every extra concurrent reader turns a sequential read into a seek storm and makes the run slower, and on a network share each thread is another round trip. **It changes only the speed: the duplicate groups, the copy kept in each, the `duplicates/NNN` numbering and the plan order are identical at any thread count.** Unset means the machine's core count capped at 8, which is why `mmm config show` prints it commented out on a machine that has not set it. `0` is refused rather than read as "decide for me" — zero means *no bound* to the underlying thread pool, which is the opposite of what the setting does — and so is anything above 1024. The storage-type guidance is in [the user guide](../USER-GUIDE.md#how-many-threads).
